...
class McpClientManager extends EventEmitter {
  ...
  async loadConfig() {
    ...
    const interpolated = servers.map(interpolateEnv);
    const baseRoot = path.resolve(path.dirname(this.configPath), '..');
    return interpolated.filter((server) => {
      if (server?.command === 'node' && Array.isArray(server.args) && server.args.length > 0) {
        const candidate = server.args[0];
        if (typeof candidate === 'string' && candidate.includes('/')) {
          const resolved = path.resolve(baseRoot, candidate);
          if (!fs.existsSync(resolved)) {
            console.warn(`[MCP] Skipping ${server.name}: missing ${resolved}`);
            return false;
          }
        }
      }
