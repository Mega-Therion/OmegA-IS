...
class LLMClient {
    ...
    async _completeOpenAI(messages, options) {
        ...
        return this._request(this.baseProtocol, this.baseUrl, this.path, headers, payload,
    ...
    async _completeGemini(messages, options) {
        ...
        return this._request(this.baseProtocol, this.baseUrl, path, { 'Content-Type': 'application/json' }, payload,
            data => data.candidates?.[0]?.content?.parts?.[0]?.text);
    }

    _request(protocol, hostname, path, headers, payload, extractor) {
