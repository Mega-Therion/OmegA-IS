...
function createMCPEnv(mcpEnv) {
  ...
  console.log(`   Configured ${Object.keys(mcpEnv).filter(k => mcpEnv[k]).length} environment variables`);
}
...
function main() {
  ...
  if (Object.keys(keys).length === 0) {
    ...
    console.warn('\n   Trying to read from process.env...');
  }
  ...
  if (configuredServers.length > 0) {
    ...
    console.log('   ⚠️  No servers configured yet (add keys to keys.env)');
  }
  ...
  console.log('\n✨ Setup complete!');
  console.log('   Next: Add your keys to keys.env in the root directory');
  console.log('   Then run: node packages/brain/mcp/setup-from-keys.mjs');
}
