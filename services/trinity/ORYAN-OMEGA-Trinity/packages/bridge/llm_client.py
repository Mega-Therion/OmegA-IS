...
class LLMClient:
    ...
    def _configure(self):
        ...
        if self.provider == "github":
            ...
            self._model = os.getenv("GEMINI_MODEL", config["model"])
            self._base_url = f"{config['base_url']}/{self._model}:generateContent?key={self._api_key}"
    ...
    def complete(self, messages: List[Dict[str, str]], **kwargs) -> Optional[str]:
        ...
        if self.provider == "azure":
            ...
            pass  # API key is in URL for Gemini
        else:
