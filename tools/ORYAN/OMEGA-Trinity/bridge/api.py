...
INTERNAL_TOKEN = os.getenv("INTERNAL_TOKEN") or os.getenv("OMEGA_INTERNAL_TOKEN")
INTERNAL_SKIP = {"/health", "/memory/status"}

@app.middleware("http")
async def enforce_internal_token(request: Request, call_next):
    if INTERNAL_TOKEN:
        if request.url.path not in INTERNAL_SKIP:
            auth_header = request.headers.get("authorization", "")
            header_token = request.headers.get("x-internal-token") or (auth_header.split(" ")[1] if " " in auth_header else auth_header)
            if header_token != INTERNAL_TOKEN:
                return JSONResponse(status_code=401, content={"ok": False, "error": "Internal token required"})
