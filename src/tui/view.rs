...
// --- MIAMI VICE / TRON NEON COLOR PALETTE ---
const HOT_PINK: Color = Color::Rgb(255, 0, 255);
const ELECTRIC_CYAN: Color = Color::Rgb(0, 255, 255);
const DEEP_PURPLE: Color = Color::Rgb(150, 0, 255);
const NEON_MINT: Color = Color::Rgb(50, 255, 180);
const NEON_AMBER: Color = Color::Rgb(255, 200, 0);
const TRON_BLUE: Color = Color::Rgb(40, 120, 255);
const SOFT_BLUE: Color = Color::Rgb(60, 160, 255);
const LIME_GREEN: Color = Color::Rgb(50, 255, 50);
const SLATE_GRAY: Color = Color::Rgb(60, 70, 90);
const DARK_BG: Color = Color::Rgb(5, 5, 15);

fn pulse_color(app: &App, a: Color, b: Color) -> Color {
    if app.spinner_index % 12 < 6 { a } else { b }
}

fn shimmer_line(text: &str, app: &App) -> Line {
...
pub fn ui(f: &mut Frame, app: &mut App) {
    ...
    let chunks = Layout::default()
        ...
        .constraints([
            ...
            Constraint::Min(3),     // Main Body (Changes based on Tab)
...
fn draw_header(f: &mut Frame, app: &App, area: Rect) {
    ...
    let brand_color = if app.status == StatusState::Working {
        ...
        pulse_color(app, ELECTRIC_CYAN, NEON_MINT)
    };
    
    let titles = vec![" [1] COCKPIT ", " [2] NEXUS "];
    let tabs = Tabs::new(titles)
        .block(Block::default().borders(Borders::ALL).border_type(BorderType::Rounded).border_style(Style::default().fg(SLATE_GRAY)))
    ...
    let branding = Line::from(vec![
        ...
        Span::styled(" SOVEREIGN ", Style::default().fg(SLATE_GRAY)),
    ]);
    f.render_widget(Paragraph::new(branding).alignment(Alignment::Center).block(Block::default().borders(Borders::ALL).border_type(BorderType::Rounded).border_style(Style::default().fg(SLATE_GRAY))), layout[1]);

    let mode_text = shimmer_line(&format!(" {} ", app.mode), app);
    f.render_widget(Paragraph::new(mode_text).alignment(Alignment::Right).block(Block::default().borders(Borders::ALL).border_type(BorderType::Rounded).border_style(Style::default().fg(SLATE_GRAY))), layout[2]);
}
    ...
    let canvas = Canvas::default()
        ...
        .paint(|ctx| {
            ...
            for (i, n) in nodes.iter().enumerate() {
                ...
                ctx.print(n.lon, n.lat, Span::styled(&n.name, Style::default().fg(glow).add_modifier(Modifier::BOLD)));
            }
            ...
            for (i, link) in links.iter().enumerate() {
                if let (Some(n1), Some(n2)) = (nodes.get(link.a), nodes.get(link.b)) {
                    ...
                    if let Some(label) = &link.label {
                        ...
                        ctx.print(mx, my, Span::styled(label, Style::default().fg(NEON_AMBER).add_modifier(Modifier::BOLD)));
                    }
                }
            }
            ...
            if let Some(link) = links.first() {
                if let (Some(n1), Some(n2)) = (nodes.get(link.a), nodes.get(link.b)) {
                    ...
                    ctx.print(x, y, Span::styled("•", Style::default().fg(ELECTRIC_CYAN)));
                }
            }
        });
...
fn draw_output(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let scroll_mode_color = if app.auto_scroll { LIME_GREEN } else { DEEP_PURPLE };
