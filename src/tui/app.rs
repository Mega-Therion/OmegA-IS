...
use crate::events::{UiEvent, StatusState, AgentInfo, SystemMetrics, TaskStatus, TaskUpdate};
...
impl App {
    ...
    pub fn on_ui_event(&mut self, event: UiEvent) {
        match event {
            ...
            UiEvent::DispatchPrompt { agent_count, summary } => {
                if agent_count == 0 {
                    self.dispatch_prompt = None;
                } else {
                    self.dispatch_prompt = Some(DispatchPromptUi { agent_count, summary });
                }
            }
        ...
        match parts.get(0).map(|s| s.trim_start_matches('/')) {
            Some("help") => {
                self.messages.push(Message {
                    text: "Available: /help, /status, /agents, /stream on|off, /mode focus|ops|showcase, /dispatch yes|no, /jobs, /revenue, /dna, /voice on|off, /listen, /devices, /skills, /mcp, /evolve, /quit".to_string(),
                    from_user: false,
                });
            }
            Some("dispatch") => {
            ...
            Some("revenue") => {
                ...
                self.messages.push(Message { text: report, from_user: false });
            }
            Some("dna") => {
                    ...
                    match *arg {
                        "on" => {
                            *(_engine.voice_enabled.lock().unwrap()) = true;
                            self.messages.push(Message { text: "Voice mode enabled.".to_string(), from_user: false });
                        }
                        "off" => {
                            *(_engine.voice_enabled.lock().unwrap()) = false;
                            ...
                            self.messages.push(Message { text: "Voice mode disabled.".to_string(), from_user: false });
                        }
                        _ => {
                            self.messages.push(Message { text: "Usage: /voice on|off".to_string(), from_user: false });
                        }
                    }
                } else {
                    let status = if *(_engine.voice_enabled.lock().unwrap()) { "enabled" } else { "disabled" };
                    self.messages.push(Message { text: format!("Voice mode is currently {}.", status), from_user: false });
                }
            }
            Some("listen") => {
                let engine = _engine.clone();
                let tx = _tx_core.clone();
                self.messages.push(Message { text: "Listening (5s)...".to_string(), from_user: false });
                tokio::spawn(async move {
                    match engine.listen(5).await {
                        Ok(text) => {
                            if !text.is_empty() {
                                ...
                                let _ = tx.send(UiEvent::Output("I didn't catch that.".to_string()));
                            }
                        }
                        Err(e) => {
            ...
            Some("skills") => {
                match _engine.modules.list_skills() {
                    Ok(skills) => {
                        if skills.is_empty() {
                            self.messages.push(Message { text: "No skills found in 'skills/' directory.".to_string(), from_user: false });
                        } else {
                    ...
                    Err(e) => {
                        self.messages.push(Message { text: format!("Error listing skills: {}", e), from_user: false });
                    }
                }
            }
            Some("quit") => {
                self.messages.push(Message { text: "Use Ctrl-C to exit.".to_string(), from_user: false });
            }
